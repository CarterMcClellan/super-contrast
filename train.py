...
def train(model_ckpt, dataset_name, text_col, label_col):
    ...
    tok_train = ds_train.map(tokenize_text, batched=True).map(
        encode_labels, batched=False
    )
    tok_valid = ds_valid.map(tokenize_text, batched=True).map(
        encode_labels, batched=False
    )
    tok_test = ds_test.map(tokenize_text, batched=True).map(
        encode_labels, batched=False
    )
    ...
    model_preds = [
        s["label"]
        for s in pipe(
            ds_test[text_column].values.tolist(), truncation=True, max_length=128
        )
    ]
    ...
    print(
        classification_report(
            ds_test[label_column].values.tolist(), model_preds, digits=3
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train a language identification model"
    )
    parser.add_argument(
        "--model_checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="papluca/language-identification",
